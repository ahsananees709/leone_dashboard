...
function DataTable({ data, setData }) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full table-auto bg-white border border-gray-200 mt-2 text-sm sm:text-base">
        <thead>
          <tr className="bg-[#605EA1] text-white">
            <th className="px-2 sm:px-4 py-2 border-b text-left">Item ID</th>
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.id} className="bg-gray-100 hover:bg-gray-200">
              ...
              <td className="px-2 sm:px-4 py-2 border-b">{item.title}</td>
              <td className="px-2 sm:px-4 py-2 text-blue-900 border-b">
                <a href={item.url} target="_blank" rel="noopener noreferrer" className="break-words">
                  {item.url}
                </a>
              </td>
              ...
              <td className="px-2 sm:px-4 py-2 border-b">
                <button
                  ...
                  className="bg-[#22177A] px-2 py-1 text-white hover:bg-[#605EA1] text-xs sm:text-sm"
                >
                  Edit Title
                </button>
              </td>
